...
def call_open_source_model(messages, n=1):

    # You need to intput the URL to call, and the verify token
    open_source_model_url = ""
    verify_token = ""
    resp = requests.post(open_source_model_url, data=json.dumps({"messages": messages, "verify_token":verify_token}), headers= {"Content-Type": "application/json"}, timeout=9.5)
    return resp
...
class Get_And_Score():
    def __init__(self):
        self.model_prompts = {
            ...
            "open_source_model":"Reference back to the question, concise, and demonstrate a clear understanding of the topic.\n\n",
    ...
    def forward(self, messages, request_str, model_name, score=False, estimated_score=0.1, filter_messages=True ):
        ...
        if "open_source_model" in model_name:
            responses = [call_open_source_model(messages_for_generation)]
...
def generate_replies(messages, request_string):
        ...
        if "Ask one relevant and insightful question about the preceding context." in  messages[-1]["content"] or "Ask a follow up question." in messages[-1]["content"]:
            ...
            all_threads.append(threading.Thread(target=response_generator.forward, args=(messages, request_string, "open_source_model",True,0.21)))
            ...
            all_threads.append(threading.Thread(target=response_generator.forward, args=(messages, request_string, "open_source_model",True,0.5)))
